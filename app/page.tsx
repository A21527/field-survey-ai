...
export default function FieldSurveyAI() {
  // Navigation Tabs
  const [activeTab, setActiveTab] = useState<'survey' | 'transcript'>('transcript'); // 'survey' or 'transcript'
  ...
  const transmitAudioPayload = async (audioBlob: Blob, fileExtension: string) => {
    try {
      ...
      formDataPayload.append('audio', audioBlob, `survey_recording.${fileExtension}`);
      ...
      setTranscriptText(`Subsystem Error: ${(err as Error).message}`);
  ...
  const extractStructuredData = async (textToParse: string) => {
    ...
    try {
      ...
      setEngineLogs((prev) => [...prev, `> Error extracting structures: ${(err as Error).message}`]);
    } finally {
      setSessionStatus('IDLE');
      setAiStatus('Ready');
    }
